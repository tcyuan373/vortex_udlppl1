        ...
        for idx, batch_manager in enumerate(to_send):
            ...
            while num_sent < batch_manager.num_queries:
                ...
                if serialize_batch_size != 1:
                    print("Warning: emit batch size is not 1 at StepA emitWorker")
                start_pos = num_sent
                end_pos = num_sent + serialize_batch_size
                serialized_batch = batch_manager.serialize(start_pos, end_pos)
                new_key = STEPA_NEXT_UDL_PREFIX + str(batch_manager.question_ids[start_pos])
...
class StepAUDL(UserDefinedLogic):
    ...
    def __init__(self,conf_str):
        ...
        self.max_emit_batch_size = 1 # emit one at a time since stepD needs to fetch by qid
